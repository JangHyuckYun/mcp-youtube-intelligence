...
from .core import collector, comments, transcript, monitor, segmenter, entities, summarizer
...
async def get_comments(
    ...
    *, config: Config, storage: BaseStorage
    ...
    # Check cache
    cached = await storage.get_comments(video_id, limit=top_n)
    if not cached:
        raw = comments.fetch_comments(video_id, max_comments=config.max_comments, yt_dlp=config.yt_dlp_path)
        if raw:
    ...
    if summarize:
        return comments.summarize_comments(cached, top_n=top_n)
