...
> 고품질 요약을 원한다면 **최소한 Local LLM (Ollama)을 권장**합니다 — **무료, 오프라인, API 키 불필요**.
>
> ```bash
...
> ```
>
> **실측 결과** (RTX 3070 8GB, 5분 19초 한국어 영상, 모델 로드 완료 상태):
>
> | 방식 | GPU 생성 시간 | 품질 | 비용 |
> |------|:------------:|:----:|:----:|
> | Extractive (LLM 없음) | 즉시 | ⭐⭐ 핵심 문장 나열 | 무료 |
> | **Ollama qwen2.5:7b** | **37초** | ⭐⭐⭐⭐ 구조화된 요약 | **무료** |
> | Ollama aya-expanse:8b | 82초 | ⭐⭐⭐ 다국어 요약 | 무료 |
> | GPT-4o / Claude | 3~5초 | ⭐⭐⭐⭐⭐ 최고 품질 | ~$0.001/영상 |
>
> ⚠️ **첫 실행 시 모델 로드에 40~70초 추가 소요**됩니다. 이후 모델이 메모리에 있으면 위 시간만 걸립니다.
...
| **초경량 (라즈베리파이)** | `qwen2.5:1.5b` | 0.9GB | ✅ 양호 | ✅ 양호 | ⭐⭐ |
...
> `keep_alive` 설정으로 모델을 메모리에 유지하면 반복 요청이 훨씬 빨라집니다.
>
> **영상 길이별 예상 시간** (qwen2.5:7b, 웜 상태):
>
> | 영상 길이 | 자막 크기 | 예상 요약 시간 |
> |:---------:|:---------:|:-------------:|
> | ~5분 | ~3,000자 | ~37초 |
> | ~10분 | ~6,000자 | ~50초 |
> | ~20분 | ~12,000자 | ~70초 |
